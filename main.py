...
from evaluation import jaccard_index
...
def train(args):
    # img_data = dset.ImageFolder(root=args.root, transform=transforms.Compose([
    #     transforms.Resize(size=args.img_size),
    #     transforms.CenterCrop(size=(args.img_size, args.img_size * 2)),
    #     transforms.ToTensor(),
    # ]))
...
def test(args, i=0):
    ...
    for (image, label) in pbar:
        ...
        jaccard += loss.sum().item()
        num_iter += 1
        pbar.set_description('Epoch {}, Iter {}, Jaccard Index: {:.5f}'.format(i + 1, num_iter, loss.item()))
    writer.add_scalars(main_tag='Testing', tag_scalar_dict={
        'Jaccard index': jaccard / len(img_batch)
    }, global_step=i )
